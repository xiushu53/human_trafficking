...
function hdi_val(val) {
    ...
    d3.json("test6.json", function(error, graph) {
        ...
        var titleDesp = [
            ...
            ["GEND_2015", "Gender Inequality Index (GII)", "A composite measure reflecting inequality in achievement between women and men in three dimensions: reproductive health, empowerment and the labour market."],
            ...
            ["LABO_2015", "Labour force participation rate, female (% ages 15 and older)", "Percentage of a countryís working-age population that engages actively in the labour market, either by working or looking for work. It provides an indication of the relative size of the supply of labour available to engage in the production of goods and services."],
        ...
        graph.nodes.forEach(function(d) {
            ...
            if (d.position == "Seller") {
                if (+d[val_list[valIdx]] > 0) {
                    suplier_data.push(+d[val_list[valIdx]])
                    both_data.push(+d[val_list[valIdx]])
                }
            } else if (d.position == "Market") {
                if (+d[val_list[valIdx]] > 0) {
                    market_data.push(+d[val_list[valIdx]])
                    both_data.push(+d[val_list[valIdx]])
                }
            }
        });

        var ave_data = [d3.mean(suplier_data), d3.mean(market_data)];
...
function position_val(pos_val) {
    ...
    if (pos_val == "continent") {
        ...
        var pos_node = document.getElementsByClassName("Seller");
        for (var i = 0; i < pos_node.length; i++) {
            pos_node[i].style.fill = "#FB8072";
        }
        var pos_node = document.getElementsByClassName("Market");
        for (var i = 0; i < pos_node.length; i++) {
